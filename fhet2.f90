subroutine fhet1(nx,ny,dx,dt,N,jfonte,ifonte,atraso,LT,cw,parq,tipo,fonte,mate)
  ...
  do k = 1,N
    ...
    if (tipo.eq.2) then !ABC 2D
      ...
      j=ny;
      PN(j,i) = PL(j,i) - cw(j-1,i) * dt/dy * (PL(j,i) - PL(j-1,i)); 
      
      j=1;
      PN(j,i) = PL(j,i) + cw(j,i) * dt/dy * (PL(j+1,i) - PL(j,i)); 
    ...
    do i=2,nx-1
      do j=2,ny-1
        C1 = mate(j,i) * GxJ(j,i) * dt2 / dx2
        C2 = mate(j,i) * GxJ(j,i-1) * dt2 / dx2 
        C3 = mate(j,i) * GIy(j,i) * dt2 / dy2
        C4 = mate(j,i) * GIy(j-1,i) * dt2 / dy2
    ...
    if (fonte.eq.1) then
      ...
      PP(jfonte,ifonte) = PP(jfonte,ifonte) + dt2 * (1.d0+2.d0 * cte1) * dexp(cte1)
    else
